...
GMlibWrapper::mousePressed(const QString& name, QMouseEvent* event ) {
    ...
    if(!_move_object_button_pressed && event->button() == Qt::RightButton)
    {
        ...
        if(obj) {
            if(!_select_multiple_objects_pressed){
...
void GMlibWrapper::mouseMoved(const QString &name, QMouseEvent *event)
{
    ...
    if(!_move_object_button_pressed && !_rotate_object_button_pressed)
    {
        const float scale = 5.0f;
        const GMlib::Vector<float,2> delta (
                    (_current_mouse_pos(0) - _prev_mouse_pos(0)) * scale / cam->getViewportW(),
                    (_prev_mouse_pos(1) - _current_mouse_pos(1)) * scale / cam->getViewportH()
                    );
        cam->move( delta );
    }

    if(_rotate_object_button_pressed)
    {
        _glsurface->makeCurrent();
