...
const override;
  ...
  //*****************************************
    ...
    template <typename T>
    inline
    PSimpleSubSurf<T>::PSimpleSubSurf( PSurf<T,3>* s, T su, T eu, T u, T sv, T ev,
T v )
    {
    ...
    template <typename T>
    inline
    PSimpleSubSurf<T>::PSimpleSubSurf( const PSimpleSubSurf<T>& copy ) :
GMlib::PSurf<T,3>( copy )
    {
    ...
    //*****************************************************
    template <typename T>
    void PSimpleSubSurf<T>::eval( T u, T v, int d1, int d2, bool /*lu*/, bool
/*lv*/) const {
      this->_p.setDim(3,3);
    ...
    template <typename T>
    inline
    void PSimpleSubSurf<T>::set(GMlib::PSurf<T,3>* s, T su, T eu, T u, T sv, T ev,
T v) {
      _s  = s;
      _su = su;
      _sv = sv;
